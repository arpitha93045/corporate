...
| POST | `/api/checkout` | **auth** | Place an order. Accepts optional `Idempotency-Key` header (up to 80 chars); replaying the same key for the same user returns the original order instead of creating a duplicate. Each `items[]` entry may carry optional `branding {message, logoUrl}` (per-line engraving text + logo URL); lines merge only when product **and** branding match. Optional top-level `paymentTerms` (`IMMEDIATE` default, or `NET_30`); `NET_30` requires a non-blank `poNumber` (≤80 chars, else 400) and places the order on invoice terms — status stays `PLACED` with `paymentStatus=INVOICED`, an `invoiceNumber` + `dueDate` (today+30) are assigned, stock is still reserved, and a plain-text invoice is emailed to the buyer. `IMMEDIATE` orders continue to the Stripe card flow. |
...
| POST | `/api/admin/orders/{orderNumber}/mark-invoice-paid` | **admin** | Settle a net-30 invoice out-of-band: flips a `NET_30` order from `PLACED` to `PAID`, sets `paidAt` + `paymentStatus=PAID`, and emails the payment-received confirmation. 409 if the order isn't net-30 or isn't `PLACED` (guards double-pay and card orders); 404 if unknown. Stripe is not involved. |
